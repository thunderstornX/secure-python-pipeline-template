...
results may differ under different versions. CI runs Semgrep in a pinned
container (\texttt{returntocorp/semgrep:1.96.0}) for reproducibility;
local runs install Semgrep unpinned (it carries heavyweight transitive
dependencies that conflict with the scanner's own requirements), so
locally reproduced finding counts may differ slightly with the installed
Semgrep version. The eval results in this paper were produced with
\texttt{semgrep 1.161.0}.
